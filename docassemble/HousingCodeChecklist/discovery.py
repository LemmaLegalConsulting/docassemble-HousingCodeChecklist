...
class DiscoveryDict(DADict):
    ...
    def init(self, *pargs, **kwargs):
        ...
        if hasattr(self, "yaml_source"):
            if not self.yaml_source.startswith(
                "/"
            ):  # A relative path using docassemble package notation
    ...
    def _load_from_yaml(self, yaml_path: str):
        ...
        for item in yaml_items:
            ...
            dict_item = self.initializeObject(
                item["name"]
    ...
    def ask_about_items(
        self, show_category_notes: bool = True, category: Optional[str] = None
    ):
    ...
    def checked_values(self):
        ...
        return DAList(
            elements=[key for key, value in self.iteritems() if value.checked is True]
        )

    def unchecked_values(self):
        """Returns a list of unchecked discovery items."""
        return DAList(
            elements=[key for key, value in self.iteritems() if value.checked is False]
        )
    ...
    def matches_from_category(self, category, checked_only=True):
        ...
        if checked_only:
            return [
                ...
                and self.elements[key].category == category
            ]
